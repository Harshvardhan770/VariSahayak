...
-- Face embeddings: no policy at all, on purpose. RLS is enabled and nothing grants access,
-- so `authenticated` and `anon` are denied every operation. Only the service role, which
-- bypasses RLS as table owner, may read or write. Embeddings never reach a client.
